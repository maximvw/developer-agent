...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("nested/dir/test_file.txt", "Content in a nested directory.", "nested_directories"),
    ]
)
def test_write_file_creates_and_writes_successfully(setup_workspace, test_path, test_content, test_id):
    """
    ...
    # 2. Проверяем результат выполнения функции
    assert result == f"Файл '{test_path}' успешно создан/перезаписан."
    
    # 3. Проверяем состояние файловой системы
...
def test_write_file_path_traversal_attack(setup_workspace):
    ...
    """
    # Попытка выйти из рабочей директории
